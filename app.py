...
@st.cache_data
def load_data():
    # change this path to your merged file
    df = pd.read_csv("employee_attrition_full.csv")
    # clean Arrow types if needed
...
if "df" not in st.session_state:
    ...
    st.session_state.baseline = (st.session_state.df['Attrition'] == 'Left').mean() * 100

df = st.session_state.df
baseline = st.session_state.baseline

# ── Global CSS (FORCED WHITE BACKGROUND) ──────────────────────────────────
st.markdown("""
<style>
...
.main.block-container { padding: 2rem 2.5rem 3rem; max-width: 1300px; background: var(--white); }
...
.insight-box { background: var(--blue-50); border-left: 4px solid var(--blue-500); border-radius: 0 10px 10px 0; padding:.9rem 1.1rem; margin:.6rem 0; font-size:.88rem; line-height:1.55; }
